...
class MomentService {
  ...
  async getSingle(momentId) {
    ...
    const statement = `
        SELECT 
        m.id id, m.content content, m.updateAt updateTime,
        JSON_OBJECT('id', u.id, 'name',u.name, 'avator', u.avator_url) userInfo,
        ...
        (SELECT IF(COUNT(l.id),JSON_ARRAYAGG(
          ...
          (SELECT JSON_ARRAYAGG(JSON_OBJECT('filename', f.filename)) FROM file f WHERE f.moment_id = m.id) filenames
    FROM moment m 
